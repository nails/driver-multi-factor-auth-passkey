...
All of the WebAuthn detail (the library, the credential store, the ceremony builders) lives in [`nails/module-auth`](https://github.com/nails/module-auth)'s `Passkey` service; this driver only adapts that service to the MFA module's `Driver` contract, using its `Interactive` companion interface to render its own challenge/setup markup instead of a numeric code field.
...
Requires `nails/module-auth` with passkey support and `nails/module-multi-factor-auth` with `Interactive` driver support.
